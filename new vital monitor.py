...
def run(
    ...
    # ── ① 서보 이동 중/직후 샘플 무효화 (motion-hold) ──
    # 카메라가 팬/틸트하면 rPPG ROI가 흔들리고, 고정 ROI 옵티컬 플로우는
    # 화면 전체의 흐름을 호흡으로 오인한다. 이동 중과 직후 샘플은 버린다.
    MOTION_HOLD_SEC = 1.5
            ...
            for frame_rgb, box in model.preview:
                ...
                if tracker is not None:
                    ...
                    if tracker.is_moving():
                        ...
                        # 이동 종료: 옵티컬 플로우 기준 프레임이 어긋났으므로
                        # 버퍼를 비우고 다음 프레임에서 앵커를 다시 잡는다
                        was_moving = False
                        need_reanchor = True
                        buf_t.clear()
                        buf_d.clear()
                        buf_v.clear()
                    motion_hold = now - last_motion_time < MOTION_HOLD_SEC
                ...
                # ── ③ 모터 이동 직후에는 앵커를 강제로 다시 잡는다 ──
                if rr_tracker.needs_anchor(now) or need_reanchor:
                ...
                if now - last_hr_update >= hr_update_interval:
                    ...
                    # ── ④ 모터 이동 중/직후 샘플은 신뢰도 0으로 게이팅 ──
                    if motion_hold:
                        vitals.hr_conf = 0.0
                        vitals.hr_reason = "MOTOR MOVING"
                        current_rr_conf = 0.0
                        rr_reason = "MOTOR MOVING"
