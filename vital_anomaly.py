...
class VitalAnomalyDetector:
    ...
    HR_RANGE = (40.0, 200.0)
    RR_RANGE = (6.0, 40.0)
    # 얼굴 피부 온도가 이 밖이면 센서 오독이거나 얼굴이 아닌 배경을 잰 것이다.
    # (실내 벽면은 보통 20~26°C 로 잡히므로 하한 30 은 배경 오매핑 검출에도 쓰인다)
    ...
    #   TEMP_CRITICAL 은 "피부 표면 온도" 기준이라 심부체온보다 낮게 잡는다.
    #   상한 37.5 는 열화상 발열 스크리닝에서 흔히 쓰는 피부온 임계이고,
    #   하한 34.0 은 저체온보다는 "얼굴이 아닌 곳을 재고 있다" 는 신호에 가깝다.
    #   ※ 실내 온도가 낮은 방에서는 정상인도 34~35°C 로 나온다. 반드시 현장 보정.
    TEMP_CRITICAL = (3.0, 37.5) #34.0, 37.5
    ...
    def __init__(self, calib_sec=180.0, min_samples=40, window=5, trigger=3,
                 ...
                 hr_sigma=2.5, rr_sigma=4.0, temp_sigma=0.4, use_temp=True):
    ...
    def push(self, hr, hr_conf, rr, rr_conf,
             temp=None, temp_conf=None, now=None):
