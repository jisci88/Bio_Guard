- 참고 오픈 소스 (References)

1. rPPG (비접촉 심박 측정)
* **저장소:** [open-rppg](https://github.com/KegangWangCCNU/open-rppg)
* **주요 참고 파일:** `example.py` (심박 추출 로직), `POS` 알고리즘 모듈
* **활용 계획:** 라즈베리 파이 5 환경에 맞게 POS 알고리즘 경량화 및 이식

2. EVM (미세 변화 시각화)
* **저장소:** [Eulerian-Video-Magnification](https://github.com/joeljose/Eulerian-Video-Magnification)
* **주요 참고 파일:** `evm.py` (핵심 증폭 로직)
* **활용 계획:** 환자 안면의 혈류 변화 시각화 및 실시간 스트리밍 연동


3. 심박수, 호흡수 정확하게 측정해주는 유료 라이브러리
* https://www.rouast.com/
* vitallens 설치, ffmpeg 설치 등 환경 구성이 불편
